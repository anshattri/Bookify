...
const Navbar = () => {
    ...
    const navItems = [
        ...
        { link: "Blog", path: "/blog" },
        { link: "Login", path: "/loguser"}

        
    ];
    return (
        <header className="w-full fixed top-0 left-0 right-0 transition-all ease-in duration-300 bg-blue-400">
            <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 " : ""
                }`}>
                <div className="flex justify-between items-center text-base gap-8">
                    <Link to="/" className="text-2xl font-bold text-black flex items-center gap-2"><FaBlog className="inline-block"/>Bookify</Link>

                    <ul className="md:flex space-x-12 hidden navitems">

                        {
                            navItems.map(({ link, path }) => <Link key={link} to={path}  className="link block text-base cursor-pointer uppercase text-white hover:text-blue-700">
                                {link}
                            </Link>)
                        }
                    </ul>
